...
const TRANSFERS = {
  ...
  aiTransfer() {
    ...
    const targets = DB.players.filter(p => {
      ...
      const st = GAME.G.pstate[p.id];
      const status = st ? (st.transferStatus || (st.listed ? 'sell' : 'normal')) : 'normal';
      if (status === 'unavailable' && U.rnd() < 0.92) return false;
      let ask = Math.max(50000, Math.round((p.value || 0) * 1.15));
      if (p.contract <= GAME.G.season + 1) ask = Math.round(ask * 0.65);
      if (p.age <= 22 && p.pot - p.ovr >= 6) ask = Math.round(ask * 1.35);
      if (status === 'sell') ask = Math.round(ask * 0.78);
      if (status === 'loan') ask = Math.round(ask * 0.9);
      if (status === 'unavailable') ask = Math.round(ask * 2.35);
      return p.ovr >= buyer.rep - 8 && p.ovr <= buyer.rep + 5 && p.value > 0 && ask < budget && p.age < 33 && seller;
    });
