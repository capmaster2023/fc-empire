...
const UI = {
  ...
  async init() {
    ...
    if (!this._clickGuardInstalled) {
      ...
      document.addEventListener('click', e => {
        const a = e.target && e.target.closest && e.target.closest('a[href="#"],a[href=""]');
    ...
    this.render(`<div class="center-screen"><div class="logo">⚽</div><h1>Football Empire</h1><p class="sub" style="color:var(--sel)">Mobile</p><p class="muted">Chargement de la base mondiale…<br>18 405 joueurs réels · 662 clubs · 51 ligues</p><div class="spinner"></div></div>`);
    await DB.load();
    this.mainMenu();
    GAMEPAD.init();
  },
  ...
  render(html, opts = {}) {
    ...
    this.app.querySelectorAll('button').forEach(b => {
      if (b.dataset.safeClick) return;
      b.dataset.safeClick = '1';
      b.addEventListener('click', e => {
        if (!b.closest('.console-nav, .hint-bar, .fm-topstrip, .fm-fly')) e.stopPropagation();
      });
    });
    ...
    const nextRoute = this.routeSnapshot ? this.routeSnapshot() : { screen: currentScreen };
    if (!this._history) this._history = [];
    if (!this._suspendHistory && this._lastRoute && this._lastRoute.screen && this._lastRoute.screen !== nextRoute.screen && this._lastRoute.screen !== 'loading') {
      this._history.push(this._lastRoute);
  ...
  routeSnapshot() {
    return {
      screen: this.screen || 'home',
      leagueId: this._leagueId,
      conf: this._conf,
      continent: this._continent,
      country: this._country,
      tableLeagueId: this._tableLeagueId,
      squadClubId: this._squadClubId,
      acadTab: this._acadTab,
      activeAcademyAid: this._activeAcademyAid,
      q: this._q,
      newsCat: this._newsCat,
      tacFrom: this._tacFrom
    };
  },

  restoreRoute(r) {
    if (!r || !r.screen) return this.home();
    this._suspendHistory = true;
    switch (r.screen) {
      case 'menu': return this.mainMenu();
      case 'wizard1': return this.wizardStart();
      case 'pickLeague': return this.pickLeague();
      case 'pickCountry': return this.pickCountry(r.continent || r.conf, r.country);
      case 'pickClub': return r.leagueId ? this.pickClub(r.leagueId) : this.pickLeague();
      case 'coachProfile': return this.coachProfile();
      case 'pickOwner': return this.pickOwner();
      case 'home': return this.home();
      case 'central': return this.central();
      case 'profile': return this.profileScreen();
      case 'club': return this.club();
      case 'finance': return this.financeScreen();
      case 'ownerInvestments': return this.ownerInvestments();
      case 'academy': return r.activeAcademyAid ? this.academyPlayer(r.activeAcademyAid) : this.academyScreen(r.acadTab || 'apercu');
      case 'squad': return this.squad(r.squadClubId);
      case 'table': return this.table(r.tableLeagueId);
      case 'world': return this.world();
      case 'transfers': return this.transfers();
      case 'search': return this.searchPlayers(r.q || '');
      case 'news': return this.newsScreen();
      case 'save': return this.saveScreen();
      case 'tactics': return this.tactics(r.tacFrom || 'club');
      default: return this.home();
    }
  },

  goBack() {
    this.closeFly && this.closeFly();
    const r = (this._history || []).pop();
    if (r) return this.restoreRoute(r);
    if (GAME && GAME.G && GAME.G.myClub !== undefined) return this.home();
    return this.mainMenu();
  },

  goStartMenu() {
    this.closeFly && this.closeFly();
    try { if (GAME && GAME.G && GAME.G.myClub !== undefined) SAVE.save(); } catch (e) {}
    this.mainMenu();
  },
  ...
  // ---------- MENU PRINCIPAL — splash FM26 ----------
  mainMenu() {
    this.screen = 'menu';
    const meta = SAVE.meta();
    this.render(`
      <div class="fm-splash">
        <div class="rib r1"></div><div class="rib r2"></div><div class="rib r3"></div>
        <div class="rib r4"></div><div class="rib r5"></div><div class="rib r6"></div>
        <div class="fm-title"><h1>Football<br>Empire</h1><span class="fm-mob">Mobile</span></div>
        <div class="fm-tiles">
          <button class="fm-tile" onclick="UI.wizardStart()"><span class="ti">⚙️</span><b>Nouvelle partie</b></button>
          <button class="fm-tile" onclick="UI.saveScreen()"><span class="ti">📂</span><b>Charger autre partie</b></button>
          <button class="fm-tile ${meta ? '' : 'off'}" onclick="UI.continueGame()">
            <span class="ti">🏳️</span>
            ${meta ? `<small>${U.esc(meta.club)} · Saison ${meta.season}</small>` : ''}
            <b>Charger partie récente</b>
          </button>
        </div>
        <div class="fm-btm">
          <button onclick="UI.saveScreen()">Paramètres</button>
          <button onclick="UI.aboutScreen()">À propos</button>
          <button onclick="UI.startSpectator()">Voir match</button>
          <button onclick="UI.toast('Bientôt disponible !')">Récompenses</button>
        </div>
        <div class="fm-banner">⚽ Football Empire — World Simulation · 18 405 joueurs réels · 662 clubs · 51 ligues</div>
      </div>`);
  },

  aboutScreen() {
    this.toast('Football Empire — 100 % simulation, 100 % hors ligne. Fait avec amour à Montréal.');
  },
  ...
  startSpectator() {
    ...
    this._history = []; this._lastRoute = null; this._lastRenderedScreen = null;
    this.home();
  },

  // ---------- WIZARD FM26 : nouvelle partie en 4 étapes ----------
  wizChrome(step, title, sub, inner, opts = {}) {
    const left = opts.leftLabel || 'Menu principal';
    const leftFn = opts.leftFn || 'UI.mainMenu()';
    return `<div class="fm-wiz">
      <div class="fm-wiz-head">
        <div><h2>${title}</h2><p class="muted small">${sub}</p></div>
        <div class="fm-steps"><span>ÉTAPE ${step} SUR 4</span><div class="fm-prog"><div style="width:${step * 25}%"></div></div></div>
      </div>
      <div class="fm-wiz-body">${inner}</div>
      <div class="fm-wiz-foot">
        <button class="fm-gbtn" onclick="${leftFn}">${left}</button>
        ${opts.confirmFn ? `<button class="fm-confirm" ${opts.confirmDisabled ? 'disabled' : ''} onclick="${opts.confirmFn}">${opts.confirmLabel || 'Confirmer'}</button>` : ''}
      </div>
    </div>`;
  },

  // Étape 1 — mode de jeu (Carrière) + type de carrière
  wizardStart() {
    this.screen = 'wizard1';
    if (!this._role || this._role === 'spectator') this._role = this._role || 'coach';
    const roles = [
      ['coach', '🧢', 'Coach', "Vous dirigez l'équipe : tactique, effectif, matchs. Le Président garde le dernier mot sur les transferts — vous devrez gagner sa confiance."],
      ['president', '👔', 'Président', 'Vous dirigez le club : budget, mercato, ventes forcées, centre de formation. Tout passe par votre bureau.'],
      ['spectator', '🌍', 'Spectateur', "Le monde vit tout seul. 51 championnats se déroulent sous vos yeux, sans que vous touchiez à rien."]
    ];
    const cur = roles.find(r => r[0] === this._role) || roles[0];
    this.render(this.wizChrome(1, 'Mode de jeu', 'Choisissez votre façon de jouer à Football Empire',
      `<div class="fm-modes">
        <button class="fm-mode sel">⚽ Carrière</button>
        <button class="fm-mode off">🎖 Défi</button>
        <button class="fm-mode off">👥 Mon club</button>
      </div>
      <div class="fm-shields">
        ${roles.map(([id, ic, lb]) => `<button class="fm-shield ${this._role === id ? 'sel' : ''}" onclick="UI._role='${id}';UI.wizardStart()"><span class="sh"><span>${ic}</span></span><b>${lb}</b></button>`).join('')}
      </div>
      <div class="fm-desc">${cur[3]}</div>`,
      {
        confirmFn: this._role === 'spectator' ? 'UI.startSpectator()' : 'UI.pickLeague()',
        confirmLabel: 'Confirmer'
      }));
  },

  // Étape 2 — environnement du jeu : continent → pays → championnat → club
  pickLeague() {
    this.screen = 'pickLeague';
    if (!this._collapsedContinents) this._collapsedContinents = {};
    const world = DB.worldHierarchy();
    const order = ['Africa', 'Europe', 'Asia', 'North America', 'South America', 'Oceania', 'Other'];
    const labels = {
      Africa: '🌍 Afrique', Europe: '🇪🇺 Europe', Asia: '🌏 Asie',
      'North America': '🌎 Amérique du Nord', 'South America': '🌎 Amérique du Sud',
      Oceania: '🌏 Océanie', Other: '📦 Non classé'
    };
    let html = '<div class="card"><b>Environnement du jeu</b><p class="muted small">Choix propre : continent → pays → championnat → club. La liste principale affiche les pays, pas des “Liga Division 1” jetées au hasard comme si la navigation était une loterie.</p></div>';
    for (const continent of order) {
      const group = world[continent];
      if (!group) continue;
      const collapsed = !!this._collapsedContinents[continent];
      const countries = Object.keys(group).sort((a, b) => a.localeCompare(b, 'fr'));
      const safeContinent = this.jsArg(continent);
      html += `<div class="section-title section-toggle"><span>${labels[continent] || continent}</span><button class="mini-toggle" onclick="UI.toggleContinent('${safeContinent}')">${collapsed ? 'Afficher' : 'Cacher'}</button></div>`;
      if (collapsed) continue;
      for (const country of countries) {
        const leagues = group[country] || [];
        const clubCount = leagues.reduce((a, L) => a + (L.nClubs || ((DB.clubsOfLeague.get(L.id) || []).length)), 0);
        const avg = leagues.length ? (leagues.reduce((a, L) => a + (L.avg || 0), 0) / leagues.length).toFixed(1) : '—';
        html += `<div class="row country-row" onclick='UI.pickCountry(${JSON.stringify(continent)},${JSON.stringify(country)})'>
          <div><b>${U.esc(country)}</b><br><span class="muted small">${leagues.length} championnat(s) · ${clubCount} clubs</span></div>
          <span class="badge" style="color:${U.ovrColor(Number(avg) || 60)}">${avg}</span></div>`;
      }
    }
    this.render(this.wizChrome(2, 'Environnement du jeu', 'Choisissez un continent, puis un pays', html,
      { leftLabel: '◀ Précédent', leftFn: 'UI.wizardStart()' }));
  },

  toggleContinent(continent) {
    if (!this._collapsedContinents) this._collapsedContinents = {};
    this._collapsedContinents[continent] = !this._collapsedContinents[continent];
    this.preserveScrollOnce();
    this.pickLeague();
  },

  // Compat anciennes sauvegardes/UI : un vieux code peut encore appeler toggleConf.
  toggleConf(conf) {
    const continent = DB.CONF_CONTINENT[conf] || conf || 'Other';
    this.toggleContinent(continent);
  },

  pickCountry(continent, country) {
    this.screen = 'pickCountry';
    this._continent = DB.CONF_CONTINENT[continent] || continent || 'Other';
    this._conf = Object.keys(DB.CONF_CONTINENT).find(k => DB.CONF_CONTINENT[k] === this._continent) || continent;
    this._country = country;
    const leagues = DB.leagues.filter(L => (L.country === country) && ((L.continent || DB.continentForCountry(L.country, L.conf)) === this._continent))
      .sort((a, b) => (a.level - b.level) || ((b.avg || 0) - (a.avg || 0)) || a.name.localeCompare(b.name, 'fr'));
    const html = `<div class="card"><b>${U.esc(country)}</b><p class="muted small">Sélectionne le championnat. Après seulement tu verras les clubs. C’est basique, donc naturellement il fallait le réparer.</p></div>` +
      leagues.map(L => `<div class="row" onclick="UI.pickClub(${L.id})">
        <div><b>${U.esc(L.name)}</b><br><span class="muted small">Division ${L.level} · ${L.nClubs || (DB.clubsOfLeague.get(L.id) || []).length} clubs</span></div>
        <span class="badge" style="color:${U.ovrColor(L.avg)}">${L.avg}</span></div>`).join('');
    this.render(this.wizChrome(2, U.esc(country), 'Choisissez le championnat', html,
      { leftLabel: '◀ Pays', leftFn: 'UI.pickLeague()' }));
  },

  pickClub(leagueId) {
    this.screen = 'pickClub';
    this._leagueId = leagueId;
    const L = DB.leagueById.get(leagueId);
    const clubs = (DB.clubsOfLeague.get(leagueId) || []).slice().sort((a, b) => b.rep - a.rep || a.name.localeCompare(b.name, 'fr'));
    let html = '';
    ...
    this.render(this.wizChrome(2, U.esc(L.name), `${U.esc(L.country)} · choisissez le club que vous allez diriger`, html,
      { leftLabel: '◀ Championnats', leftFn: `UI.pickCountry('${this.jsArg(this._continent || L.continent || DB.continentForCountry(L.country, L.conf))}', '${this.jsArg(this._country || L.country)}')` }));
  },
  ...
  // Étape 3 — profil entraîneur / dirigeant (FM26 : infos + photo)
  NATION_EXTRAS: ['Burundi', 'Rwanda', 'Canada', 'RD Congo', 'Cameroun', 'Sénégal', "Côte d'Ivoire", 'Haïti', 'Belgique', 'Suisse', 'Maroc', 'Algérie', 'Tunisie', 'Nigeria', 'Ghana'],
  nations() {
    const set = new Set(this.NATION_EXTRAS);
    for (const L of DB.leagues) if (L.country) set.add(L.country);
    return [...set].sort((a, b) => a.localeCompare(b, 'fr'));
  },

  savedProfile() {
    try { return JSON.parse(localStorage.getItem('fe_profile_v1')); } catch { return null; }
  },

  coachProfile() {
    this.screen = 'coachProfile';
    if (!this._draft) {
      this._draft = { first: '', last: '', nation: 'Canada', birth: '1995-01-01', style: 'Général', rep: 'Nationale', photo: null, useExisting: false };
    }
    const D = this._draft;
    const saved = this.savedProfile();
    const roleLbl = this._role === 'president' ? 'dirigeant' : 'entraîneur';
    const styles = ['Général', 'Offensif', 'Défensif', 'Formation des jeunes'];
    const reps = ['Locale', 'Nationale', 'Reconnue (continentale)', 'Mondiale'];
    const photoHtml = `
      <div class="fm-photowrap">
        <div class="fm-photo" onclick="UI.takePhoto(true)">
          ${D.photo ? `<img src="${D.photo}" alt="">` : '👤'}
          <span class="cam">📷</span>
        </div>
        <div class="fm-photo-actions">
          <button onclick="UI.takePhoto(true)">📷 Prendre une photo</button>
          <button onclick="UI.takePhoto(false)">🖼 Choisir dans la galerie</button>
          ${D.photo ? `<button onclick="UI._draft.photo=null;UI.coachProfile()">✕ Retirer la photo</button>` : ''}
        </div>
      </div>`;
    const createPanel = `
      <div class="fm-panelbox ${!D.useExisting ? 'sel' : ''}">
        <div class="fm-prow" onclick="UI._draft.useExisting=false;UI.coachProfile()">
          <span class="fm-check">✓</span><h3>Créer un nouveau profil</h3>
        </div>
        ${!D.useExisting ? `
          <div style="margin-top:12px">
            ${photoHtml}
            <div class="fm-field"><label>Prénom</label><input value="${U.esc(D.first)}" oninput="UI._draft.first=this.value" placeholder="Prénom"></div>
            <div class="fm-field"><label>Nom</label><input value="${U.esc(D.last)}" oninput="UI._draft.last=this.value" placeholder="Nom"></div>
            <div class="fm-field"><label>Nationalité</label>
              <select onchange="UI._draft.nation=this.value">${this.nations().map(n => `<option ${n === D.nation ? 'selected' : ''}>${U.esc(n)}</option>`).join('')}</select></div>
            <div class="fm-field"><label>Date de naissance</label><input type="date" value="${D.birth}" onchange="UI._draft.birth=this.value"></div>
          </div>` : ''}
      </div>`;
    const existingPanel = saved ? `
      <div class="fm-panelbox ${D.useExisting ? 'sel' : ''}">
        <div class="fm-prow" onclick="UI.useExistingProfile()">
          <span class="fm-check">✓</span>
          <div class="fm-photo" style="width:58px;height:58px;font-size:22px;cursor:default">${saved.photo ? `<img src="${saved.photo}" alt="">` : '👤'}</div>
          <div><h3>Utiliser un profil existant</h3><span class="muted small">${U.esc(saved.first)} ${U.esc(saved.last)} · ${U.esc(saved.nation)}</span></div>
        </div>
      </div>` : '';
    const rightPanel = `
      <div class="fm-panelbox">
        <div class="fm-field"><label>Style d'entraînement</label>
          <div class="fm-irow"><span class="fm-i">i</span><div><select onchange="UI._draft.style=this.value">${styles.map(s => `<option ${s === D.style ? 'selected' : ''}>${s}</option>`).join('')}</select></div></div></div>
        <div class="fm-field"><label>Réputation</label>
          <div class="fm-irow"><span class="fm-i">i</span><div><select onchange="UI._draft.rep=this.value">${reps.map(r => `<option ${r === D.rep ? 'selected' : ''}>${r}</option>`).join('')}</select></div></div></div>
        <div class="fm-field"><label>Titres d'${roleLbl}</label>
          <div class="fm-irow"><span class="fm-i">i</span><div><select disabled><option>Rien de sélectionné</option></select></div></div></div>
      </div>`;
    this.render(this.wizChrome(3, `Profil ${roleLbl}`, `Choisissez ou créez les détails de votre profil ${roleLbl}`,
      `<div class="fm-cols">
        <div class="fm-col">${createPanel}${existingPanel}</div>
        <div class="fm-col">${rightPanel}</div>
      </div>`,
      {
        leftLabel: '◀ Sélectionner environnement du jeu',
        leftFn: `UI.pickClub(${this._leagueId})`,
        confirmFn: 'UI.confirmProfile()',
        confirmLabel: 'Confirmer'
      }));
  },
  ...
  useExistingProfile() {
    const saved = this.savedProfile();
    if (!saved) return;
    this._draft = { ...saved, useExisting: true };
    this.coachProfile();
  },

  // Capture photo (caméra frontale ou galerie), redimensionnée en 256×256
  takePhoto(fromCamera, target) {
    const inp = document.createElement('input');
    inp.type = 'file'; inp.accept = 'image/*';
    if (fromCamera) inp.capture = 'user';
    inp.onchange = () => {
      ...
      fr.onload = () => {
        ...
        img.onload = () => {
          ...
          if (target === 'game' && GAME.G && GAME.G.profile) {
            GAME.G.profile.photo = url;
            try { const s = this.savedProfile(); if (s) { s.photo = url; localStorage.setItem('fe_profile_v1', JSON.stringify(s)); } } catch {}
            SAVE.save();
            this.profileScreen();
          } else {
            if (!this._draft) this._draft = {};
            this._draft.photo = url;
            this.coachProfile();
          }
        };
        img.src = fr.result;
  ...
  confirmProfile() {
    const D = this._draft || {};
    if (!D.useExisting && !(D.first || '').trim() && !(D.last || '').trim()) {
      return this.toast('Entre au moins un prénom ou un nom, coach.');
    }
    const credMap = { 'Locale': 35, 'Nationale': 45, 'Reconnue (continentale)': 60, 'Mondiale': 75 };
    this._profile = {
      first: (D.first || '').trim(), last: (D.last || '').trim(),
      nation: D.nation || 'Canada', birth: D.birth || '',
      style: D.style || 'Général', rep: D.rep || 'Nationale',
      photo: D.photo || null, credStart: credMap[D.rep] || 45
    };
    this.pickOwner();
  },

  // Étape 4 — propriétaire du club
  pickOwner() {
    this.screen = 'pickOwner';
    const c = DB.clubById.get(this._clubId);
    const base = FINANCE.initBudgets()[c.id] || 0;
    if (!this._ownerType) this._ownerType = 'normal';
    const rich = this._ownerType === 'rich';
    this.render(this.wizChrome(4, 'Propriétaire du club', `${U.esc(c.name)} — choisissez le départ financier du club`,
      `<div class="fm-shields">
        <button class="fm-shield ${!rich ? 'sel' : ''}" onclick="UI._ownerType='normal';UI.pickOwner()"><span class="sh"><span>🏦</span></span><b>Normal</b></button>
        <button class="fm-shield ${rich ? 'sel' : ''}" onclick="UI._ownerType='rich';UI.pickOwner()"><span class="sh"><span>💎</span></span><b>Riche</b></button>
      </div>
      ${!rich ? `<div class="fm-desc">Budget réaliste du club, aucune injection spéciale. Progression plus lente, plus propre, plus dure.<br><b class="money">Budget estimé : ${U.money(base)}</b></div>`
        : `<div class="fm-desc">Injection dès le début dans les finances globales : stade, formation, staff, médical, scouting, marketing. Le mercato reste limité par le fair-play financier.</div>
           <div class="chips wrap" style="justify-content:center;margin-top:12px">${OWNER.AMOUNTS.map(m => `<button class="chip ${(UI._ownerAmount || 100000000) === m ? 'active' : ''}" onclick="UI._ownerAmount=${m};UI.pickOwner()">${U.money(m)}</button>`).join('')}</div>`}`,
      {
        leftLabel: '◀ Précédent',
        leftFn: 'UI.coachProfile()',
        confirmFn: `UI.startWithOwner('${this._ownerType}', ${rich ? (this._ownerAmount || 100000000) : 0})`,
        confirmLabel: 'Commencer la carrière'
      }));
  },

  pickOwnerAmount() { this.pickOwner(); },

  startWithOwner(type, amount) {
    GAME.newGame(this._clubId, this._role || 'coach', { type, injection: amount || 0 });
    if (this._profile) {
      GAME.G.profile = this._profile;
      if (GAME.G.role === 'coach') GAME.G.coachCredibility = this._profile.credStart || 45;
      try { localStorage.setItem('fe_profile_v1', JSON.stringify(this._profile)); } catch {}
      const nm = `${this._profile.first} ${this._profile.last}`.trim();
      if (nm) NEWS.add(`👤 ${nm} (${this._profile.nation}) prend ses fonctions. Réputation : ${this._profile.rep}.`, 'club');
    }
    this._draft = null;
    this._history = []; this._lastRoute = null; this._lastRenderedScreen = null;
    SAVE.save();
  ...
  // ---------- PROFIL EN JEU (style FM26) ----------
  profileScreen() {
    this.screen = 'profile';
    const G = GAME.G;
    const P = G.profile || {};
    const my = G.myClub ? DB.clubById.get(G.myClub) : null;
    const isOwnerRole = G.role === 'president';
    const roleTitle = isOwnerRole ? 'Propriétaire' : 'Entraîneur';
    const name = `${P.first || ''} ${P.last || ''}`.trim() || (isOwnerRole ? 'Propriétaire' : 'Coach');
    const age = P.birth ? Math.floor((Date.now() - new Date(P.birth).getTime()) / 3.15576e10) : null;
    this.render(this.shell('profile',
      `<div class="panel-wrap"><div class="panel">
        <div class="fm-profile-hero">
          <div class="fm-photo" onclick="UI.takePhoto(true,'game')">${P.photo ? `<img src="${P.photo}" alt="">` : '👤'}<span class="cam">📷</span></div>
          <div>
            <h2>${U.esc(name)}</h2>
            <div class="csub">${U.esc(P.nation || '—')}${age ? ` · ${age} ans` : ''} · ${roleTitle}</div>
            ${my ? `<div class="muted small">${U.esc(my.name)}</div>` : ''}
          </div>
        </div>
        <div class="section-title">${isOwnerRole ? 'Profil propriétaire' : 'Profil entraîneur'}</div>
        <div class="stat-row"><span>${isOwnerRole ? 'Style de gestion' : "Style d'entraînement"}</span><b>${U.esc(P.style || 'Général')}</b></div>
        <div class="stat-row"><span>Réputation</span><b>${U.esc(P.rep || 'Nationale')}</b></div>
        ${G.role === 'coach' ? `<div class="stat-row"><span>Crédibilité</span><div class="bar"><div style="width:${G.coachCredibility}%;background:var(--purple2)"></div></div><b>${G.coachCredibility}/100</b></div>` : ''}
        ${my ? `<div class="stat-row"><span>Budget club</span><b class="money">${U.money(GAME.budget(my.id))}</b></div>` : ''}
        <div class="section-title">Options</div>
        <button class="btn" onclick="UI.takePhoto(true,'game')">📷 Reprendre une photo de profil</button>
        <button class="btn" onclick="UI.takePhoto(false,'game')">🖼 Choisir une photo dans la galerie</button>
        <button class="btn btn-ghost" onclick="UI.saveScreen()">💾 Sauvegardes & options</button>
      </div></div>`,
      { back: 'UI.home()' }));
  },

  // ---------- SHELL CONSOLE FC26 ----------
  // Sidebar FM26 : rail vertical gauche + flyouts + bandeau date/continuer
  nav(active) {
    const G = GAME.G;
    const my = G && G.myClub ? DB.clubById.get(G.myClub) : null;
    const P = G && G.profile;
    const unread = G ? Math.min(9, (G.offers || []).length + (G.pendingTransfers || []).length + Math.min((G.news || []).length, 5)) : 0;
    const ini = my ? U.esc(my.name.split(/\s+/).map(w => w[0]).join('').slice(0, 2).toUpperCase()) : 'FE';
    const clubGrp = ['home', 'central', 'squad', 'academy', 'table', 'club'].includes(active);
    const schGrp = ['transfers', 'search'].includes(active);
    const flyRow = (icon, label, fn) => `<button class="fm-fly-row" onclick="UI.closeFly();${fn}"><span class="ficon">${icon}</span>${label}</button>`;
    const avatar = P && P.photo ? `<img class="fm-ava" src="${P.photo}" alt="">` : '👤';
    const coachName = P ? U.esc(`${P.first || ''} ${P.last || ''}`.trim()) : '';
    const isOwnerRole = G && G.role === 'president';
    const profileRoleTitle = isOwnerRole ? 'Propriétaire' : 'Entraîneur';
    const profileRoleLabel = isOwnerRole ? 'Profil propriétaire' : 'Profil entraîneur';
    return `<nav class="console-nav">
      <button class="fm-it" title="Retour" onclick="UI.goBack()">‹</button>
      <button class="fm-it" onclick="UI.closeFly();UI.newsScreen()">✉️${unread ? `<span class="fm-badge">${unread}</span>` : ''}</button>
      <button class="fm-it ${active === 'home' ? 'act' : ''}" onclick="UI.closeFly();UI.home()">🏠</button>
      ${my ? `<button class="fm-it ${clubGrp ? 'act' : ''}" onclick="UI.toggleFly('club')"><span class="fm-crest">${ini}</span></button>` : ''}
      <button class="fm-it ${active === 'profile' ? 'act' : ''}" onclick="UI.toggleFly('coach')">${avatar}</button>
      <button class="fm-it ${active === 'world' ? 'act' : ''}" onclick="UI.closeFly();UI.world()">🌍</button>
      <button class="fm-it ${schGrp ? 'act' : ''}" onclick="UI.toggleFly('search')">🔍</button>
      <span class="fm-spring"></span>
      <button class="fm-it" title="Menu de départ" onclick="UI.goStartMenu()">🏁</button>
      <button class="fm-it" onclick="UI.closeFly();UI.saveScreen()">⚙️</button>

      <div class="fm-scrim" onclick="UI.closeFly()"></div>
      ${my ? `<div class="fm-fly" data-fly="club">
        <div class="fm-fly-head"><span class="fm-crest">${ini}</span><div><b>${U.esc(my.name)}</b><small>Infos</small></div></div>
        ${flyRow('①', 'Équipe 1ère', 'UI.squad()')}
        ${flyRow('📋', 'Rapport équipe', 'UI.central()')}
        ${flyRow('⛺', 'Centre de dév.', 'UI.academy()')}
        ${flyRow('📅', 'Calendrier & Compét.', `UI.table(${my.league})`)}
        ${flyRow('📈', 'Finances', 'UI.financeScreen()')}
        ${flyRow('🏛', 'Bureau', 'UI.club()')}
      </div>` : ''}
      <div class="fm-fly" data-fly="coach">
        <div class="fm-fly-head">${P && P.photo ? `<img class="fm-ava" src="${P.photo}" style="width:34px;height:34px" alt="">` : '<span class="fm-crest">👤</span>'}<div><b>${coachName || profileRoleTitle}</b><small>${P ? U.esc(P.nation || '') : ''}</small></div></div>
        <div class="fm-fly-lbl">${profileRoleTitle}</div>
        ${flyRow('👤', profileRoleLabel, 'UI.profileScreen()')}
        ${flyRow('🏆', 'Vitrine des trophées', 'UI.club()')}
        ${flyRow('💾', 'État de la partie', 'UI.saveScreen()')}
        ${flyRow('🚪', 'Menu principal', 'UI.goStartMenu()')}
      </div>
      <div class="fm-fly" data-fly="search">
        <div class="fm-fly-head"><span class="fm-crest">🔍</span><div><b>Recherche</b><small>Mercato & scouting</small></div></div>
        ${flyRow('🔎', 'Rech. de joueur', "UI.searchPlayers('')")}
        ${flyRow('🔁', 'Transferts', 'UI.transfers()')}
        ${flyRow('🌍', 'Monde', 'UI.world()')}
      </div>

      ${G && G.myClub !== undefined ? `<div class="fm-topstrip">
        <div class="fm-datebox"><small>Saison ${G.season}</small><b>${U.fmtDateShort(G.day, G.season)}</b></div>
        <button class="fm-continue" onclick="UI.continueDay()">Continuer ▸</button>
      </div>` : ''}
    </nav>`;
  },

  toggleFly(name) {
    const flys = document.querySelectorAll('.fm-fly');
    let opened = false;
    flys.forEach(f => {
      const willOpen = f.dataset.fly === name && !f.classList.contains('open');
      f.classList.toggle('open', willOpen);
      if (willOpen) opened = true;
    });
    const sc = document.querySelector('.fm-scrim');
    if (sc) sc.classList.toggle('open', opened);
  },
  closeFly() {
    document.querySelectorAll('.fm-fly').forEach(f => f.classList.remove('open'));
    const sc = document.querySelector('.fm-scrim');
  ...
  hintBar(opts = {}) {
    const G = GAME.G;
    const back = 'UI.goBack()';
    const extra = opts.extra || '';
    const meta = G ? `${U.fmtDateShort(G.day, G.season)} · S${G.season}` : '';
    return `<div class="hint-bar">
      <span class="sysmeta">${G && G.myClub ? `<span class="money">${U.money(GAME.budget(G.myClub))}</span>` : ''}<span>${meta}</span></span>
      ${extra}
      <span class="spacer"></span>
      <button class="hb-btn" onclick="UI.closeFly();UI.newsScreen()">Boîte de réception</button>
      <button class="hb-btn" onclick="UI.closeFly();${back}">◀ Précédent</button>
      <button class="hb-btn primary" onclick="UI.closeFly();UI.continueDay()">▶ Suivant</button>
    </div>`;
  },
  ...
  header(title) {
    ...
    return `<div class="topbar">
      ...
      <div class="hdr-right">${my ? `<b>${U.esc(my.name)}</b><br><span class="small">${G.role === 'president' ? 'Propriétaire' : 'Coach'}${G.role === 'coach' ? ' · Créd. ' + G.coachCredibility + '/100' : ''} · <span class="money">${U.money(GAME.budget(my.id))}</span></span>` : '<b>Spectateur</b>'}</div>
    </div>`;
  },
    ...
    if (my) {
      cards.push({
        ...
        body: `<div class="stat-row"><span>Budget</span><b class="money">${U.money(GAME.budget(my.id))}</b></div>
          <div class="stat-row"><span>Rôle</span><b>${G.role === 'president' ? 'Propriétaire' : 'Coach'}</b></div>
  ...
  liveMatch() {
    // APK Android : moteur C++ NDK via Capacitor. Navigateur/dev : fallback JS spatial.
    if (window.MatchEngine && MatchEngine.isNativeAvailable && MatchEngine.isNativeAvailable()) return this.liveMatchNative();
    return this.liveMatchJs();
  },

  async liveMatchNative() {
    const entry = this._pending;
    clearInterval(this._liveInt);
    if (this._liveRaf) cancelAnimationFrame(this._liveRaf);
    this._nativeLive = true;
    this.screen = 'live';
    const h = DB.clubById.get(entry.m.h), a = DB.clubById.get(entry.m.a);
    const matchData = MatchEngine.buildFromEntry(entry);
    let state;
    try {
      state = await MatchEngine.startMatch(matchData);
    } catch (e) {
      console.warn('[MatchEngine] fallback JS, le plugin natif n’a pas répondu', e);
      this._nativeLive = false;
      return this.liveMatchJs();
    }
    const mySide = entry.m.h === GAME.G.myClub ? 'home' : 'away';
    ...
    this.render(`${this.header('Match en direct')}
      <div class="content live-content">
        <div class="scorebug live-scorebug">
          <div class="sb-comp">${U.esc(entry.comp)} · moteur C++ NDK + Canvas</div>
          <div class="sb-line">
            <span class="sb-team">${U.esc(h.name)}</span>
            <b class="sb-score" id="liveScore">0 : 0</b>
            <span class="sb-team tr">${U.esc(a.name)}</span>
          </div>
          <div class="sb-clock" id="liveMin">0' · possession <span id="livePoss">50</span>% · <span id="liveBallOwner">coup d'envoi</span></div>
        </div>
        <div class="live-stage" id="liveStage">
          <canvas id="liveRadar" class="live-radar" aria-label="Radar tactique du match"></canvas>
          <div class="live-hook" id="jumpHook">Moteur natif actif</div>
        ...
        <div class="live-thumbbar speed">
          ...
          <button class="btn-mini live-speed" onclick="UI.setLiveSpeed(4,this)">x4</button>
        ...
        <div class="live-drawer" id="liveTacticsPanel">
          <div class="live-drawer-head"><b>Consignes natives</b><button class="btn-mini" onclick="UI.toggleLiveTactics(false)">✕</button></div>
          <p class="muted small">Les consignes partent au C++ et le Canvas reçoit seulement l’état du match. Oui, enfin un ballon qui ne se téléporte pas comme s’il fuyait les impôts.</p>
    ...
    const drawRadar = () => {
      if (this.screen !== 'live' || !this._nativeLive) return;
      const st = MatchEngine.state || state;
      ...
      ctx.fillStyle = '#101720'; ctx.fillRect(0, 0, w, hgt);
      for (let i = 0; i < 10; i++) {
      ...
      for (const x of (st.players || [])) {
        ...
        const py = pad + (x.y / 100) * (hgt - pad * 2);
        const r = Math.max(4.2, Math.min(w, hgt) * 0.014);
        ctx.beginPath(); ctx.arc(px, py, r, 0, Math.PI * 2);
        ctx.fillStyle = x.team === 'home' ? '#F2F4F8' : '#F0524A'; ctx.fill();
        ctx.lineWidth = x.hasBall ? 3 : 1; ctx.strokeStyle = x.hasBall ? '#A78BFA' : 'rgba(0,0,0,.55)'; ctx.stroke();
        if (x.stamina < 38) { ctx.beginPath(); ctx.arc(px, py, r + 3, 0, Math.PI * 2); ctx.strokeStyle = 'rgba(242,199,75,.7)'; ctx.lineWidth = 1.5; ctx.stroke(); }
      }
      const bx = pad + ((st.ball?.x || 50) / 100) * (w - pad * 2);
      const by = pad + ((st.ball?.y || 50) / 100) * (hgt - pad * 2);
      ctx.beginPath(); ctx.arc(bx, by, Math.max(3.2, Math.min(w, hgt) * 0.010), 0, Math.PI * 2);
      ctx.fillStyle = '#FFD54A'; ctx.fill(); ctx.shadowColor = '#FFD54A'; ctx.shadowBlur = 10; ctx.fill(); ctx.shadowBlur = 0;
      this._liveRaf = requestAnimationFrame(drawRadar);
    };

    const addEvent = e => {
      if (!e || !e.text) return;
      const box = this.el('commentary'); if (!box) return;
      const d = document.createElement('div');
      const t = String(e.type || '').toLowerCase();
      d.className = 'news-item' + (t === 'goal' ? ' goal' : t === 'save' || t === 'shot' ? ' goal' : '');
      d.textContent = `${Math.max(1, Math.round(e.time || 0))}' ${e.text}`;
      box.prepend(d); if (box.children.length > 42) box.lastChild.remove();
      if (t === 'goal') { const stage = this.el('liveStage'); if (stage) { stage.classList.remove('goal-flash'); void stage.offsetWidth; stage.classList.add('goal-flash'); } }
    };

    let last = performance.now();
    let busy = false;
    const simDur = Math.max(1, this._simDur || 3);
    this._liveInt = setInterval(async () => {
      if (this.screen !== 'live' || !this._nativeLive) return clearInterval(this._liveInt);
      if (busy) return;
      const now = performance.now();
      const matchDelta = ((now - last) / 1000) * (90 / (simDur * 60));
      last = now; busy = true;
      try {
        state = await MatchEngine.tick(matchDelta);
        const e = state.event;
        const lastLogged = MatchEngine.eventLog[MatchEngine.eventLog.length - 1];
        if (lastLogged === e) addEvent(e);
        if (this.el('liveScore')) this.el('liveScore').textContent = `${state.score.home} : ${state.score.away}`;
        if (this.el('liveMin')) this.el('liveMin').firstChild.textContent = `${Math.floor(state.time)}' · possession `;
        if (this.el('livePoss')) this.el('livePoss').textContent = state.possession.home;
        const owner = (state.players || []).find(p => p.hasBall);
        if (this.el('liveBallOwner')) this.el('liveBallOwner').textContent = owner ? `${owner.name} (${owner.role})` : 'ballon libre';
        if (state.status && state.status.over) {
          clearInterval(this._liveInt); if (this._liveRaf) cancelAnimationFrame(this._liveRaf);
          const res = MatchEngine.resultFor(entry);
          GAME.recordMyMatch(entry, res);
          SAVE.save();
          this._nativeLive = false;
          setTimeout(() => { if (this.screen === 'live') this.postMatch(res); }, 800);
        }
      } catch (e) {
        console.warn('[MatchEngine] tick failed', e);
      } finally { busy = false; }
    }, 50);
    ...
    canvas.onclick = (ev) => {
      const st = MatchEngine.state || state;
      const rect = canvas.getBoundingClientRect();
      const pad = Math.max(14, Math.min(rect.width, rect.height) * 0.045);
      const fx = U.clamp((ev.clientX - rect.left - pad) / (rect.width - pad * 2) * 100, 0, 100);
      const fy = U.clamp((ev.clientY - rect.top - pad) / (rect.height - pad * 2) * 100, 0, 100);
      let best = null, bd = 999;
      for (const pl of (st.players || [])) { const d = Math.hypot(pl.x - fx, pl.y - fy); if (d < bd) { bd = d; best = pl; } }
      if (best && bd < 9) this.toast(`${best.name} · ${best.role} · ${Math.round(best.stamina)}% · ${best.action || 'shape'}`);
    };
    drawRadar();
  },
  ...
  liveMatchJs() {
    ...
    const drawPlayer = (x, teamKey, w, h, pad) => {
      ...
      ctx.strokeStyle = isHolder ? '#A78BFA' : 'rgba(0,0,0,.55)';
      ctx.stroke();
      if (x.fit < 38) {
    ...
    const drawRadar = () => {
      ...
      ctx.fill();
      ctx.shadowColor = '#FFD54A'; ctx.shadowBlur = 10; ctx.fill(); ctx.shadowBlur = 0;
    ...
    const phaseMs = () => (S.durMs / 94) / 5 / (S.mul || 1);
    ...
    // Tap sur le radar : identifie le joueur le plus proche du doigt (nom · poste · énergie)
    canvas.onclick = (ev) => {
      const rect = canvas.getBoundingClientRect();
      const pad = Math.max(14, Math.min(rect.width, rect.height) * 0.045);
      const fx = U.clamp((ev.clientX - rect.left - pad) / (rect.width - pad * 2) * 100, 0, 100);
      const fy = U.clamp((ev.clientY - rect.top - pad) / (rect.height - pad * 2) * 100, 0, 100);
      const n = LIVE.nearestPlayer(fx, fy);
      if (n && n.d < 9) {
        const T = LIVE.T(n.key);
        this.toast(`${n.p.p.name} (${T.name}) · ${n.p.p.pos} · ${Math.round(n.p.fit)}% énergie · ${n.p.action}`);
      }
    };
    drawRadar();
  },
  ...
  setLiveSpeed(m, btn) {
    if (this._nativeLive && window.MatchEngine) MatchEngine.setSpeed(m).catch(() => {});
    else if (LIVE.S) LIVE.S.mul = m;
  ...
  applyLivePreset(styleKey) {
    if (this._nativeLive && window.MatchEngine) {
      const side = this._pending && this._pending.m.h === GAME.G.myClub ? 'home' : 'away';
      MatchEngine.updateTactics(side, { styleKey, styleName: styleKey }).catch(() => {});
      document.querySelectorAll('.live-tac-chip').forEach(b => b.classList.toggle('active', b.dataset.style === styleKey));
      this.toast('Consigne native appliquée : ' + styleKey.replace('_', ' '));
      return;
    }
  ...
  togglePause(force) {
    if (this._nativeLive && window.MatchEngine && MatchEngine.state) {
      const paused = force === true ? true : !(MatchEngine.state.status && MatchEngine.state.status.paused);
      (paused ? MatchEngine.pause() : MatchEngine.resume()).catch(() => {});
      MatchEngine.state.status = MatchEngine.state.status || {};
      MatchEngine.state.status.paused = paused;
  ...
  async skipLive() {
    if (this._nativeLive && window.MatchEngine) {
      clearInterval(this._liveInt);
      if (this._liveRaf) cancelAnimationFrame(this._liveRaf);
      await MatchEngine.stopMatch().catch(() => null);
      const res = MatchEngine.resultFor(this._pending);
      GAME.recordMyMatch(this._pending, res);
      SAVE.save();
      this._nativeLive = false;
      return this.postMatch(res);
    }
    if (!LIVE.S) return;
    clearInterval(this._liveInt);
    if (this._liveRaf) cancelAnimationFrame(this._liveRaf);
  ...
  world() {
    ...
    this.render(`${this.header('Le Monde')}
      <div class="content">
        <button class="btn" onclick="UI.leagueList()">🌍 Tous les championnats (${DB.leagues.length.toLocaleString('fr-FR')})</button>
    ...
    this.render(`${this.header('Club')}
      <div class="content">
        ...
        <div class="card"><div class="section-title">Centre de formation</div>
          ...
          <button class="btn btn-primary" onclick="UI.academyScreen('finance')">💶 Finance du centre de formation</button>
          <button class="btn" onclick="UI.academyScreen('scouting')">🛰️ Section scouting</button>
          <button class="btn" onclick="UI.academyScreen('developpement')">🌱 Centre de développement</button>
        </div>
        ...
        <div class="section-title">Aperçu du centre de développement</div>
  ...
  academyUpgrade() {
    ...
    SAVE.save(); this.academyScreen('finance');
  },
  ...
  academyScreen(tab) {
    ...
    if (!this._acadTab) this._acadTab = 'finance';
    if (this._acadTab === 'centre') this._acadTab = 'developpement';
    if (this._acadTab === 'apercu') this._acadTab = 'finance';
    ...
    const tabs = [['finance', 'Finance centre'], ['scouting', 'Scouting'], ['developpement', `Développement (${centre.length})`], ['cibles', `Cibles (${scouted.length})`], ['categories', 'U6-U19'], ['u19', 'U19 & Réserve']];
    ...
    const reportsHtml = (A.reports || []).slice(0, 18).map(r => {
    ...
    const financeInfraRows = Object.entries(ACADEMY.INFRA_DEFS).map(([k, d]) => {
      const lvl = I[k] || 1;
    ...
    let body = '';
    if (this._acadTab === 'finance') {
      const quality = this.academyQuality(GAME.G.myClub);
      const cost = this.academyUpgradeCost(GAME.G.myClub);
      const avgPot = centre.length ? Math.round(centre.reduce((a, y) => a + y.pot, 0) / centre.length) : 0;
      body = `
        <div class="card win">
          <div class="section-title">Finance du centre de formation</div>
          <div class="stat-row"><span>Budget club disponible</span><b class="money">${U.money(GAME.budget(GAME.G.myClub))}</b></div>
          <div class="stat-row"><span>Niveau du centre</span><b>${quality}/100</b></div>
          <div class="bar"><div style="width:${quality}%;background:${U.ovrColor(quality)}"></div></div>
          <p class="muted small" style="margin:8px 0">Ici seulement : l’investissement du centre de formation. Plus de bouton perdu au milieu du club comme une chaussette dans une laveuse.</p>
          ${GAME.G.role === 'president' ? `<button class="btn btn-primary" onclick="UI.academyUpgrade()">🏗️ Investir dans le centre — ${U.money(cost)}</button>` : `<button class="btn" onclick="UI.requestYouthScouting()">📝 Demander un budget formation au Président</button>`}
        </div>
        <div class="card">
          <div class="section-title">Résumé sportif</div>
          <div class="kpi-grid">
            <div class="kpi"><b>${centre.length}</b><span>jeunes signés</span></div>
            <div class="kpi"><b>${scouted.length}</b><span>cibles suivies</span></div>
            <div class="kpi"><b>${avgPot || '—'}</b><span>potentiel moyen</span></div>
          </div>
        </div>
        <div class="section-title">Infrastructures financées</div>
        ${financeInfraRows}`;
    } else if (this._acadTab === 'scouting') {
      ...
      body = `
        <div class="card win">
          <div class="section-title">Section scouting</div>
          ...
          <p class="muted small" style="margin:8px 0">Les rapports des scouts sont regroupés ici. Étrangement, c’est plus simple quand les choses sont rangées là où leur nom l’indique.</p>
        ...
        <div class="section-title">Rapports des scouts</div>
        ${reportsHtml || '<div class="muted small" style="padding:0 4px 8px">Aucun rapport pour l’instant.</div>'}
    ...
    } else if (this._acadTab === 'developpement') {
      body = `${sortBar}
        <div class="card"><div class="section-title">Centre de développement</div><p class="muted small">Ici vivent les jeunes que tu as déjà signés. C’est donc le vrai centre de développement, pas la salle d’attente du scouting.</p></div>
        ${centre.slice(0, 80).map(rowOf).join('') || '<div class="muted small" style="padding:8px 4px">Aucun jeune signé. Va dans Scouting ou Cibles pour en trouver.</div>'}`;
    } else if (this._acadTab === 'cibles') {
      body = `${sortBar}
        <div class="card"><div class="section-title">Cibles détectées</div><p class="muted small">Joueurs repérés mais pas encore signés. Approche, séduis, puis offre. Le rituel administratif, parce que le football adore les formulaires.</p></div>
        ${scouted.slice(0, 80).map(rowOf).join('') || '<div class="muted small" style="padding:8px 4px">Aucune cible. Lance une mission de scouting.</div>'}`;
      ...
      body = `<div class="card win"><div class="section-title">Équipe U19 / réserve</div><div class="kpi-grid"><div class="kpi"><b>${u19.length}</b><span>joueurs</span></div><div class="kpi"><b>${goals}</b><span>buts</span></div><div class="kpi"><b>${Math.round(u19.reduce((a,y)=>a+(y.morale||60),0)/Math.max(1,u19.length))}</b><span>moral moy.</span></div></div><p class="muted small">Un match jeunes est simulé chaque mois. Minutes, fatigue, moral, école et blessures influencent la progression.</p></div>${rows || '<div class="muted small">Aucun joueur U19 ou réserve.</div>'}`;
    } else {
      this._acadTab = 'finance';
      return this.academyScreen('finance');
    }
  ...
  saveScreen() {
    ...
    const meta = SAVE.meta();
    const inGame = !!GAME.G;
    this.render(`<div class="topbar"><button class="btn-back" onclick="UI.goBack()">‹</button><h2>Sauvegardes & Options</h2></div>
      <div class="content">
        ...
        <button class="btn btn-ghost" onclick="UI.goStartMenu()">🏠 Menu principal</button>
