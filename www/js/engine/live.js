...
const LIVE = {
  ...
  step() {
    ...
    S.tick++;
    S.phase++;
    if (S.phase % 5 === 0) this.minuteTick();
    if (S.over) return;
  ...
  updateTargets(key) {
    ...
    for (const x of active) {
      ...
      // Course sur passe : le destinataire attaque le point de chute au lieu d'attendre
      if (S.ball.state === 'pass' && S.ball.targetTeam === key && T.xi[S.ball.targetIdx] === x) {
        x.action = 'chase';
        x.tx = U.clamp(S.ball.x + S.ball.vx * 2.2, 3, 97);
        x.ty = U.clamp(S.ball.y + S.ball.vy * 2.2, 5, 95);
        x._speedMul = U.clamp(0.9 + pace / 100, 1, 1.95);
        continue;
      }
      ...
      if (hasBall) {
        ...
        const groupPush = x.p.group === 'AT' ? 23 : x.p.group === 'MF' ? 12 : 4;
        ...
        if (shouldPress) {
          ...
          // Marquage individuel : les défenseurs prennent l'attaquant adverse le plus
          // menaçant dans leur zone quand le ballon approche du tiers défensif.
          const dangerZone = T.isHome ? ballX < 42 : ballX > 58;
          x.mark = null;
          if (x.p.group === 'DF' && dangerZone) {
            const OT = this.T(oppKey);
            let best = null, bd = 999;
            for (const o of this.alive(OT)) {
              if (o.p.group === 'GK' || o.p.group === 'DF') continue;
              const d = this.dist(x.x, x.y, o.x, o.y);
              const threat = d - (o.p.group === 'AT' ? 6 : 0);
              if (threat < bd && d < 20) { bd = threat; best = o; }
            }
            if (best) {
              x.mark = best;
              x.action = 'mark';
              const markQ = (this.attr(x.p, 'marking', this.attr(x.p, 'def', 68)) + this.attr(x.p, 'positioning', 68)) / 140;
              // se place entre l'attaquant et son propre but, collé selon la qualité de marquage
              const gx = ownGoal, tight = 1.4 + (1 - U.clamp(markQ, 0.6, 1.3)) * 2.2;
              const ddx = gx - best.x, ddy = 50 - best.y;
              const dd = Math.sqrt(ddx * ddx + ddy * ddy) || 1;
              x.tx = U.clamp(best.x + ddx / dd * tight, 3, 97);
              x.ty = U.clamp(best.y + ddy / dd * tight * 0.5, 5, 95);
            }
          }
          if (!x.mark) {
            x.action = 'repli';
  ...
  movePlayers(key) {
    ...
    for (const x of this.alive(T)) {
      ...
      const actionBoost = x.action === 'press' ? 1.16 : x.action === 'chase' ? 1.20 : x.action === 'mark' ? 1.06 : x.action === 'run' ? 1.10 : x.action === 'carry' ? 0.95 : 1;
      ...
      const drain = (x.action === 'press' ? 0.014 : x.action === 'run' ? 0.011 : 0.006) + Math.max(0, inst.fat || 0) / 4200;
  ...
  tryInterception() {
    ...
    const tgt = B.targetTeam ? this.T(B.targetTeam).xi[B.targetIdx] : null;
    const markedBonus = tgt && nearest.p.mark === tgt ? 0.12 : 0;
    const pInt = U.clamp(0.26 + (tack - passQuality) / 150 + (DT.instructions.def - 1) * 0.18 + markedBonus, 0.08, 0.62);
  ...
  checkPressureOnHolder() {
    ...
    if (!nearest || nearest.d > 1.9) return;
    ...
    const pWin = U.clamp(0.16 + (tackle - dribble) / 180, 0.06, 0.45);
    if (U.rnd() >= pWin) return;
    {
  ...
  decideWithBall() {
    ...
    const central = Math.abs(holder.y - 50) < 25;
    const inBox = distGoal < 21 && central;
    const finalThird = AT.isHome ? holder.x > 68 : holder.x < 32;

    if (finalThird && S.tick - S.lastChanceTick > 14 && U.rnd() < 0.05 + AT.tac.atk * 0.03) {
      S.lastChanceTick = S.tick;
      this.log(`🔥 Occasion dangereuse pour ${AT.name}.`, 'chance', { club: AT.id, teamKey: atkKey, pid: holder.p.id });
    }

    if (inBox && U.rnd() < 0.58 + (holder.p.group === 'AT' ? 0.22 : 0)) return this.shoot(atkKey, holder, false);
    if (distGoal < 34 && U.rnd() < 0.15 + this.attr(holder.p, 'longshots', 62) / 650) return this.shoot(atkKey, holder, true);

    // Appel de balle : un coéquipier lancé + de l'espace = passe en profondeur devant lui
    const through = this.findThroughRunner(atkKey, holder);
    if (through && U.rnd() < (finalThird ? 0.38 : 0.24) + (this.attr(holder.p, 'vision', 68) - 68) / 200 + (AT.instructions.passingRisk || 0.5) * 0.22) {
      if (U.rnd() < 0.30) this.log(`🎯 ${holder.p.name} lance ${through.x.p.name} en profondeur !`, 'info', { club: AT.id });
      return this.passTo(atkKey, holder, through, true);
    }
  ...
  // Cherche un attaquant lancé ('run') avec de l'espace devant lui pour une passe en profondeur
  findThroughRunner(atkKey, holder) {
    const T = this.T(atkKey), dir = T.isHome ? 1 : -1;
    let best = null, bs = -999;
    T.xi.forEach((x, i) => {
      if (x.off || x === holder || x.p.group === 'GK') return;
      if (x.action !== 'run' && x.p.group !== 'AT') return;
      const ahead = (x.x - holder.x) * dir;
      if (ahead < 4) return;
      const aheadPoint = U.clamp(x.x + dir * 8, 3, 97);
      const space = this.spaceAround(this.opp(atkKey), aheadPoint, x.y);
      const pace = this.attr(x.p, 'pace', 70);
      const score = ahead * 0.5 + space * 1.4 + pace / 8 + U.gauss(0, 3);
      if (space > 5.5 && score > bs) { bs = score; best = { x, i }; }
    });
    return best;
  },
  ...
  passTo(atkKey, holder, target, depth = false) {
    const S = this.S, AT = this.T(atkKey), B = S.ball;
    const pas = this.attr(holder.p, 'pas', 68);
    const vision = this.attr(holder.p, 'vision', 68);
    const quality = (pas * 0.68 + vision * 0.32) * this.eff(holder) * (depth ? 0.92 : 1);
    AT.passTot++;

    const lead = AT.isHome ? 1 : -1;
    const leadDist = depth ? 7 + this.attr(target.x.p, 'pace', 70) / 18 : U.clamp((AT.instructions.passingRisk || 0.5) * 4, 1, 5);
    const tx = U.clamp(target.x.x + lead * leadDist, 2, 98);
