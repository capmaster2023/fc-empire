...
const GAME = {
  ...
  newGame(myClubId, role = 'coach', ownerSetup = null) {
    ...
    NEWS.add(`🏟️ Saison ${this.G.season}/${this.G.season + 1} : le monde du football s'éveille. ${DB.leagues.length.toLocaleString('fr-FR')} championnats, ${DB.clubs.length.toLocaleString('fr-FR')} clubs, ${DB.players.length.toLocaleString('fr-FR')} joueurs.`, 'monde');
